...
public class CourseDAO extends DBContext {
    ...
    public boolean mentorOfCourseCheck(int course_id, int user_id) {
     ...
     // Method to enroll a user in a course //hien tai dang k xai dc
    ...
    public void insertUserProgressAndEnrollment(UserCourseProgressDBO progressObj, EnrollmentDBO enrollment) throws SQLException {
        PreparedStatement psEnrollment = null;
        PreparedStatement psUserCourseProgress = null;
        ...
        try {
            ...
            // Insert into Enrollment table
            String sqlEnrollment = "INSERT INTO Enrollment (user_id, course_id, enrollment_date) VALUES (?, ?, ?)";
            psEnrollment = connection.prepareStatement(sqlEnrollment);
            psEnrollment.setInt(1, enrollment.getUser_id());
            psEnrollment.setInt(2, enrollment.getCourse_id());
            psEnrollment.setDate(3, new java.sql.Date(enrollment.getEnrollment_date().getTime()));
            psEnrollment.executeUpdate();

            // Insert into UserCourseProgress table
            String sqlUserCourseProgress = "INSERT INTO UserCourseProgress (user_id, course_id, completion_date, progress) VALUES (?, ?, ?, ?)";
            psUserCourseProgress = connection.prepareStatement(sqlUserCourseProgress);
            psUserCourseProgress.setInt(1, progressObj.getUser_id());
            psUserCourseProgress.setInt(2, progressObj.getCourse_id());
            psUserCourseProgress.setDate(3, new java.sql.Date(progressObj.getComplete_date().getTime()));
            psUserCourseProgress.setInt(4, progressObj.getProgress());
            psUserCourseProgress.executeUpdate();
            ...
            if (psUserCourseProgress != null) {
                try {
                    psUserCourseProgress.close();
                } catch (SQLException e) {
                    e.printStackTrace();
                }
            }
